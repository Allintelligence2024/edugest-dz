...
- Bloqué (résolu) : 2 runs CI rouges à `composer install` (6-8 s) + Pre-Deploy rouge. Cause : `$lectureDossier` manquant dans le `use` du groupe parent `core.php` → boot Laravel explosé (variable indéfinie). php-parser ne voit pas ça (syntaxe valide). Fix : 1 ligne. Leçon : toute variable ajoutée dans un fichier de routes doit être traçée dans TOUS les `use` imbriqués ; la CI backend reste le seul garde-fou. Suite : 9/10 tests verts, 1 échec UUID (`find()` sur chaîne non-UUID → 22P02) — fix `Str::isUuid` + test durci, run final en cours.
