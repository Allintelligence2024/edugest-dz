...
### Code
- [x] **P1-C0 (ajouté le 11 sept.) Accès dossier parent — 403 silencieux corrigé.** Constat : `GET eleves/{id}/{notes,presences,bulletins}` derrière `$lectureEleves` SANS parent, `{id}/paiements` SANS parent → 403 avalés par le mobile (`catch {}`), écrans vides. Fix : `$lectureDossier` (+parent) sur le dossier, +parent sur paiements ; filiation imposée par `verifierPerimetreEleve` (déjà en place) ; liste globale toujours interdite (test verrou). 6 tests : `AccesDossierParentTest`.
- [x] **P1-C1 Réécriture `mobile/src/screens/enseignant/NotesScreen.js`.** État actuel : URL prod en dur, token via AsyncStorage, header `X-Tenant-ID` maison — contourne AuthContext/SecureStore/tenant. Réécriture sur le client API officiel (`api/endpoints.js` + token SecureStore via AuthContext). Parcours : mes groupes → mes évaluations → saisie notes → sauvegarde. Tester : saisie 5 notes, rechargement, déconnexion/reconnexion. (Fait le 11 sept. : réécriture sur `enseignantApi` + SecureStore, 0 URL en dur, 0 AsyncStorage auth, 3 tests — voir § Modifications.)
- [x] **P1-C2 Brancher `mobile/src/screens/parent/BulletinsScreen.js`.** État actuel : placeholder `noData`. Brancher : liste bulletins de l'enfant (`GET eleves/{id}/bulletins`) + ouverture PDF (endpoint bulletin PDF existant). Tester avec 2 bulletins réels.
- [x] **P1-C3 Dashboard parent : brancher ou retirer.** Si branchage > 1 jour → retirer de la navigation pilote et rediriger l'accueil parent vers Notes. Aucun `-` visible le jour de la démo.
- [x] **P1-C4 Vérifier `PaiementsScreen` parent.** Liste factures de l'enfant + statuts + reçus. SATIM : bouton paiement en ligne **masqué** pour le pilote (pas d'homologation) — mention "paiement au secrétariat". Vérifier statuts `payée/émise/en_retard/partielle/annulée` mappés à l'API réelle.
- [x] **P1-C5 Vérifier planning parent + appel prof.** `parent/PlanningScreen` (planning de l'enfant) et `enseignant/PresencesScreen` (appel par séance) de bout en bout contre l'API. Noter tout écart 428/403 (rôles, tenant) et corriger côté backend **uniquement si bloquant**.
- [x] **P1-C6 Tests existants verts.** `mobile npm test` (4 suites) + `frontend npx vitest run` verts. Aucune régression. Pas de nouveaux tests sauf sur code réécrit (P1-C1 : 3 tests min : chargement groupes, saisie, erreur réseau). (Mobile : 24/24 en local le 11 sept. ; backend : via CI PR #91.)
- **2026-09-11 (P1-C2/C3/C4/C5) :** 8 écrans réécrits sur contrats réels (parent : Notes/Présences/Paiements/Bulletins/Planning/Dashboard ; prof : Planning/Presences-appel) + `EnfantContext` + `GET parents/mes-enfants` + `GET planning?eleve_id` (garde périmètre, inscriptions validées) ; Messages/Notifications masqués ; PDF bulletin via `/storage` + Linking (signé post-pilote) ; SATIM remplacé par « paiement au secrétariat ». Cause racine n°2 : intercepteur axios déballe `response.data` (convention `body?.data`, jamais `res?.data?.data`) — TOUT le code mobile rogue affichait vide. Limitations pilote assumées : planning prof = toutes les séances du tenant (lecture seule, scoping P3) ; dashboard prof hors scope. 8 tests backend + 10 tests mobile (8 fichiers).
...
### Code
- [ ] **P2-C1 Rôle `eleve` + scoping backend.** Constat P0 : le rôle `eleve` n'est seedé nulle part (rôles : super_admin, admin, gestionnaire, enseignant, parent, comptable, secretariat). Donc : (1) ajouter `eleve` à `RolePermissionSeeder` (+ permissions lecture seule notes/bulletins/planning) ; (2) scope : un élève ne lit que `eleve_id = moi` (résolu via `user->eleve_id`, jamais via paramètre client). Fail-closed : sans `eleve_id` → 403. Tests backend : élève lit ses notes (200), élève lit celles d'un autre (403), sans eleve_id (403). (Rôle seedé le 11 sept. — voir § Modifications ; reste : middleware `eleve` sur lectures dossier + cas `eleve` dans `PerimetreAccesService::elevesAutorises` + `EleveNavigator`.)
...
- **2026-09-11 (P1-C0) :** fix 403 parent silencieux sur le dossier élève (notes/présences/bulletins/paiements) : `$lectureDossier` +parent, filiation au contrôleur, liste globale verrouillée — 6 tests. Cause racine des écrans mobiles vides.
- **2026-09-11 (P1-C1/C6) :** écran notes prof réécrit sur client officiel (0 URL en dur, 0 AsyncStorage auth), 3 tests ; mobile 24/24 en local ; PHP 5/5 syntaxe OK.
- **2026-09-11 (fix 1+2+3) :** rôle `eleve` seedé (5 permissions lecture, test) ; commande `tenant:configurer-pilote` + 4 tests (fail-open contourné sans flip global — flip = post-pilote) ; `docs/ci-declencheurs.patch` préparé (`git apply --check` OK, à appliquer depuis poste autorisé — PR #91 assure la CI en attendant).
- **2026-09-11 (incidents CI) :** (1) `$lectureDossier` manquant dans le `use` du groupe parent `core.php` → boot Laravel explosé → `composer install` rouge en 6 s + Pre-Deploy rouge (2 runs). php-parser ne détecte pas (syntaxe valide) ; PHPStan non plus (routes hors analyse). Leçon : tracer toute variable route dans TOUS les `use` imbriqués. (2) `Tenant::find()` sur ID non-UUID → 22P02 : `Str::isUuid` dans la commande + test durci. Verdict final : backend ✅ (10/10 nouveaux tests, 0 régression), frontend ✅, Pre-Deploy ✅, qualité rouge pré-existant + 1 annotation ajoutée puis corrigée (`@return list<string>`).
