...
const StarIcon = ({ filled }) => (
  <svg width="12" height="12" viewBox="0 0 24 24" fill={filled ? 'var(--orange)' : 'none'} stroke={filled ? 'var(--orange)' : 'var(--muted)'} strokeWidth="2">
...
const NIVEAUX = ['1AP','2AP','3AP','4AP','5AP','1AM','2AM','3AM','4AM','1AS','2AS','3AS'];

const selectStyle = {
  background: 'var(--input-bg)', border: 'none', borderRadius: '8px',
  color: 'var(--text)', padding: '10px 12px', fontSize: '0.75rem',
};
...
export default function MarketplacePageCentres() {
  ...
  const fetchFeatured = async () => {
    try {
      const res = await api('/marketplace/featured');
  ...
  const rechercher = async () => {
    ...
    try {
      ...
      const res = await api(`/marketplace/recherche?${params.toString()}`);
  ...
  const fetchProfil = async (tenantId) => {
    try {
      const res = await api(`/marketplace/centres/${tenantId}`);
      if (res.success) setSelectedCentre(res.data);
    } catch { /* ignore */ }
  };
  ...
  const Stars = ({ note }) => (
    <div style={{ display:'flex', gap:'2px' }}>
      ...
      <span style={{ fontSize:'11px', color:'var(--muted)', marginLeft:'4px' }}>{note}</span>
    </div>
  );
    ...
    >
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:'4px', marginBottom:'8px' }}>
        {(centre.matieres_enseignees || []).slice(0, 4).map(m => (
          <span key={m} style={{
            background:'rgba(37,99,235,0.15)', color:'var(--accent)', fontSize:'9px',
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        {centre.accepte_essai_gratuit && (
          <span style={{ background:'rgba(16,185,129,0.15)', color:'var(--green)', fontSize:'9px',
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', color:'var(--text)', padding:'24px' }}>
      ...
      <div style={{ marginBottom:'28px' }}>
        <h1 style={{ fontSize:'22px', fontWeight:900, color:'var(--text)', marginBottom:'4px' }}>
          &#127963; Centres — Marketplace EduGest DZ
        </h1>
      ...
      <div style={{
        ...
        <div style={{ marginTop:'10px', display:'flex', alignItems:'center', gap:'12px' }}>
          ...
          <input type="number" placeholder="Tarif max (DA/h)"
            ...
            style={{ background:'var(--input-bg)', border:'none', borderRadius:'6px',
              color:'var(--text)', padding:'6px 10px', fontSize:'11px', width:'140px' }} />
        </div>
      </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'16px' }}>
              <div>
                <h2 style={{ fontSize:'18px', fontWeight:900, color:'var(--text)', marginBottom:'4px' }}>
            ...
            {selectedCentre.description && (
              <p style={{ fontSize:'12px', color:'var(--muted)', marginBottom:'16px', lineHeight:1.7 }}>
                {selectedCentre.description}
              </p>
            )}
              ...
              <div style={{ marginBottom:'16px' }}>
                ...
                {selectedCentre.offres.map(o => (
                  <div key={o.id} style={{
                    ...
                    <div>
                      <div style={{ fontSize:'12px', fontWeight:700, color:'var(--text)' }}>{o.titre}</div>
                      <div style={{ fontSize:'10px', color:'var(--muted)' }}>{o.matiere} · {o.type}</div>
                    </div>
                    <div style={{ textAlign:'right' }}>
                      ...
                      {o.essai_gratuit && (
                        <span style={{ fontSize:'9px', background:'rgba(16,185,129,0.15)', color:'var(--green)',
            ...
            {selectedCentre.avis?.length > 0 && (
              <div>
                ...
                {selectedCentre.avis.slice(0,3).map(a => (
                  <div key={a.id} style={{
                    background:'rgba(16,185,129,0.05)', borderRadius:'8px', padding:'10px 12px', marginBottom:'6px',
                  }}>
                    ...
                    {a.commentaire && (
                      <p style={{ fontSize:'11px', color:'var(--muted)', margin:0 }}>{a.commentaire}</p>
                    )}
                  </div>
                ))}
              </div>
