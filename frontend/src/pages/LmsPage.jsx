...
export default function LmsPage() {
  ...
  const creerCours = async () => {
    ...
    setSaving(false);
    if (res.success) { setShowNew(false); loadCours(); setMsg('Cours créé'); setMsgOk(true); }
  ...
  return (
    <div style={{ padding:'24px', background:'#070B14', minHeight:'100vh' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px' }}>
        <div>
          <h1 style={{ fontSize:'22px', fontWeight:900, color:'#fff' }}><BookOpen size={22} aria-hidden='true' />LMS — Cours en ligne</h1>
      ...
      {stats && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'12px', marginBottom:'24px' }}>
          <St label="Cours publiés"    value={stats.total_cours}       color="#2563EB" icon={<BookOpen size={12} aria-hidden="true" />} />
          <St label="Élèves inscrits"  value={stats.total_inscrits}    color="#10B981" icon={<GraduationCap size={12} aria-hidden="true" />} />
          <St label="Cours complétés"  value={stats.cours_completes}   color="#7C3AED" icon={<CheckCircle size={12} aria-hidden="true" />} />
          <St label="Certificats"      value={stats.certificats}       color="#F59E0B" icon={<Award size={12} aria-hidden="true" />} />
        </div>
      )}

      <div style={{ display:'flex', gap:'4px', marginBottom:'20px' }}>
        {[['catalogue', <BookOpen size={12} aria-hidden="true" />, 'Catalogue'],['activite', <BarChart3 size={12} aria-hidden="true" />, 'Activité récente']].map(([id,icon,label]) => (
              ...
              <div style={{ padding:'14px' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'6px' }}>
                  ...
                  <span style={{ background:c.publie?'#10B98122':'#64748b22', color:c.publie?'#10B981':'#94A3B8', fontSize:'9px', fontWeight:700, padding:'2px 8px', borderRadius:'20px', flexShrink:0 }}>
                    {c.publie ? <><CheckCircle size={9} aria-hidden='true' />Publié</> : <><Circle size={9} aria-hidden='true' />Brouillon</>}
                  </span>
                </div>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:'10px', color:'#64748B', marginBottom:'10px' }}>
                  <span><User size={16} aria-hidden='true' /> {c.inscriptions_count ?? 0}inscrits</span>
                  <span><BookOpen size={16} aria-hidden='true' /> {c.nb_lecons ?? 0}leçons</span>
      ...
      {tab === 'activite' && (
        <div style={{ background:'#0D1117', border:'1px solid #1E2D40', borderRadius:'14px', overflow:'hidden' }}>
          <div style={{ padding:'14px 20px', borderBottom:'1px solid #1E2D40', fontSize:'13px', fontWeight:700, color:'#fff' }}>
            <BarChart3 size={13} aria-hidden='true' />Activité récente des élèves
                      </div>
          {(stats?.activite_recente || []).length === 0 ? (
            ...
            <div key={i} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'12px 20px', borderBottom:'1px solid #1E2D4044' }}>
              ...
              <span style={{ fontSize:'10px', color:p.completee?'#10B981':'#64748B', fontWeight:700 }}>
                {p.completee ? <><CheckCircle size={10} aria-hidden='true' />Complété</> : <><Hourglass size={10} aria-hidden='true' />En cours</>}
              </span>
            </div>
          ))}
        </div>
      ...
      {showNew && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.7)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }} onClick={() => setShowNew(false)}>
          <div style={{ background:'#111318', border:'1px solid #1E2D40', borderRadius:'16px', padding:'24px', width:'520px', maxWidth:'90%' }} onClick={e=>e.stopPropagation()}>
            <h3 style={{ color:'#fff', fontWeight:800, marginBottom:'20px' }}><BookOpen size={16} aria-hidden='true' />Nouveau cours LMS</h3>
            {[
            ...
            <div style={{ marginBottom:'14px', fontSize:'11px', color:'#64748B', background:'#1E293B', borderRadius:'8px', padding:'10px' }}>
              <BookOpen size={11} aria-hidden='true' />Après la création → ajouter des chapitres, des leçons, des quiz et des devoirs.
                          </div>
            <div style={{ display:'flex', gap:'10px' }}>
              ...
              <button onClick={creerCours} disabled={saving || !form.titre} style={{ flex:2, background:'linear-gradient(135deg,#2563EB,#1d4ed8)', color:'#fff', border:'none', borderRadius:'8px', padding:'10px', cursor:'pointer', fontWeight:700 }}>
                {saving ? 'Création...' : <><CheckCircle size={16} aria-hidden='true' />Créer le cours</>}
              </button>
            </div>
          </div>
        </div>
